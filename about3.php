...
if(isset($_REQUEST["btnsend"]))
{
    ...
    $qry="insert into message(m_message,m_subject,u_id,u_name,u_email,u_image)values('".$message."','".$subject."','".$id."','".$name."','".$email."','".$image."')";
...
<div style="float:left;height:820px;width:820px;background:-webkit-linear-gradient(right,#4c42be,#1a118a,#050133);position:relative;bottom:50px">
...
<div class="d4" style="background-color:black;width:100%;height:45px;color:black;position:relative; top:800px">
    <center><span style="color:whitesmoke;font-size:25px;padding:5px;position:relative;bottom:850px;text-align:center;"> @Travel Beyond Limits From India.All right Reserved</span></center>
    </div>
    

        </div>
        <div style="background-color:orange;width:100%;height:650px;margin-top:120px; ">
            <span style="font-size:35px;position:relative;left:20px;top:20px;">
                 <?php
      include 'dbcon1.php';
      $qry22="select * from message join reply on reply.r_id=message.m_id";
      $result22=  mysql_query($qry22, $con);
      while ($row22= mysql_fetch_array($result22))
      {
       echo '<span style="font-size:35px;position:relative;left:20px;top:20px;">';
      echo'<img src="upload/'.$row22["u_image"].'" class="i7">';
      echo"Message By ";
      echo $row22["u_name"];
      echo '<span style="padding-left:20px;"><br><br>';
       echo $row22["m_message"];
       echo '</span>';
       echo'<br><br>';
